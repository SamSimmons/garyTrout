...
/***/ function(module, exports, __webpack_require__) {
	...
	xhr.get('localhost:3001/', function (err, data) {
	  console.log(data);
	});
	...
	var drawMarker = function drawMarker(trout, el) {
	  el.append("circle").attr('class', 'marker-out ' + trout.id).attr("cx", trout.x).attr("cy", trout.y).attr("r", 40);

	  el.append("circle").attr('class', 'marker ' + trout.id).attr("cx", trout.x).attr("cy", trout.y).attr("r", 5);
	};

	//will need a function for adding individual trout to db
	var addTrout = function addTrout(coords) {
	  var newTrout = {
	    x: coords[0],
	    y: coords[1],
	    id: Date.now(),
	...
	var removeSingleTrout = function removeSingleTrout(id) {
	...
	var fetchTrout = function fetchTrout() {};

	//will need a function for drawing a new trout to map
	var drawTrout = function drawTrout() {};

	//will need a function for drawing all selected trout to the map
	var drawAllTrout = function drawAllTrout() {
	  arr.forEach(function (trout) {
	    drawMarker(trout, map);
	  });
	};

	//appends the map to the page
	var map = d3.select("body").append("svg").attr('id', 'rotoma').attr("width", width).attr("height", height);
	...
	var stripId = function stripId(classes) {
	  return classes.split(' ').splice(1).toString();
	};

	var turnOnDeleteMode = function turnOnDeleteMode() {
	  map.on('click', null);
...
/***/ function(module, exports, __webpack_require__) {
	...
	var once = __webpack_require__(3)
